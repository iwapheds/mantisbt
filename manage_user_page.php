...
$t_query = 'SELECT COUNT(*) AS new_user_count FROM {user}
	WHERE ' . db_helper_compare_days( db_param(), 'date_created', '<=', $t_days_old );
...
if( $f_filter === 'ALL' ) {
	...
	$t_where = db_helper_compare_days( db_param(), 'date_created', '<=', $t_days_old );
...
if( 0 == $c_hide_inactive ) {
	...
	$t_query = 'SELECT count(*) as user_count FROM {user}
			WHERE ' . $t_where . ' AND ' . db_helper_compare_days( db_param(), 'last_visit', '<', $t_days_old )
			. $t_show_disabled_cond;
	$t_where_params[] = db_now();
}
